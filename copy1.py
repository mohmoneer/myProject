...
class Player(Entity):
    ...
    def update(self):
        ...
        if self.intersects(apple):
            ...
            self.new_body = Entity(parent=field,model='cube',z=-0.029,scale=(0.04,0.04,1),shader="lit_with_shadows_shader")# position=(z,x,y) z : hight new_body
            # add the head snake
...
s = Button(text="BY THABET-ALSOHIPY", color=color.black, scale=.050 ,position=(-.76,0.48))
s.fit_to_text()
...
apple = Entity(parent=field,model='sphere',color=color.red,scale=(0.05,0.05),
